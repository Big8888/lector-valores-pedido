...
function buildToppingsSummary(toppings = []) {
  ...
  return toppings
    .map((t) => `${safeNumber(t.quantity || 1)}x ${t.name || "Sin nombre"}`)
    .join(" | ");
}
...
function countKeywords(orderDetails, keywords) {
  ...
  for (const item of orderDetails) {
    ...
    if (keywords.some((k) => combined.includes(normalizeText(k)))) {
...
function interpretOlaClickPayload(payload = {}) {
  ...
  const detailRows = orderDetails.map((item) => {
    ...
    return {
      ...
      toppingsCount: toppings.reduce((acc, t) => acc + safeNumber(t.quantity || 1), 0),
    ...
    alerta: detectAlerts(
      [
        ...
        ...orderDetails.flatMap((item) =>
          Array.isArray(item?.product?.toppings) ? item.product.toppings.map((t) => t.name || "") : []
        )
      ].join(" | ")
    ),
...
module.exports = { interpretOlaClickPayload };
